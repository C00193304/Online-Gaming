	...
	: m_ipAddress(ipAddress), m_port(port), MessageReceived(handler)
{
	...
	m_sock.sin_port = htons(54010);
	m_sock.sin_addr.S_un.S_addr = INADDR_ANY; // Could also use inet_pton .... 

	bind(m_listener, (sockaddr*)&m_sock, sizeof(m_sock));
...
void CTcpListener::Run()
{
		...
		for (int i = 0; i < socketCount; i++)
		{
			...
			else // It's an inbound message
			{
				...
				else
				{
					...
					{
						SOCKET outSock = m_masterSet.fd_array[i];
	...
	while (m_masterSet.fd_count > 0)
	{
		...
		SOCKET sock = m_masterSet.fd_array[0];
